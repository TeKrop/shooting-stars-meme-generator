...
- **`server/server.ts`** — the entire backend, built around `Bun.serve()`:
  ...
  - `'/export/*'` renders the shooting-stars animation server-side as an MP4 (`server/export.ts`, `@napi-rs/canvas` + a single `ffmpeg` process, no headless browser) — see `server/keyframes.ts` below for the animation data it's built from.
